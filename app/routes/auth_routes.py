...
def login():
    ...
    if form.validate_on_submit():
        ...
        if attempt and attempt.blocked_until and attempt.blocked_until > now:
            time_left = int((attempt.blocked_until - now).total_seconds())
            flash(f'Per daug neteisingų bandymų. Bandykite po {time_left//60} min. {time_left%60} s.', 'danger')
        ...
        if not attempt:
            attempt = LoginAttempt(username=username, attempts=1, last_attempt=now)
